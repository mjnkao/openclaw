...
describe("agentCommand", () => {
  ...
  it("persists a direct ingress turn with one terminal durable outcome", async () => {
    await withTempHome(async (home) => {
      ...
      const previousDurableRuntime = process.env.OPENCLAW_DURABLE_RUNTIME;
      const previousDurableWorker = process.env.OPENCLAW_DURABLE_WORKER;
      ...
      try {
        ...
        if (previousDurableRuntime === undefined) {
          delete process.env.OPENCLAW_DURABLE_RUNTIME;
        } else {
          process.env.OPENCLAW_DURABLE_RUNTIME = previousDurableRuntime;
        }
        if (previousDurableWorker === undefined) {
          delete process.env.OPENCLAW_DURABLE_WORKER;
        } else {
          process.env.OPENCLAW_DURABLE_WORKER = previousDurableWorker;
        }
