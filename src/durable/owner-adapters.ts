...
export const subagentRunsOwnerAdapter: DurableOwnerAdapter = {
  ...
  listAttentionFacts(options): DurableOwnerAttentionFact[] {
    return listSubagentRunsFromSqlite(options).flatMap((entry) => {
  ...
  async dispatchAttention({ wake }): Promise<DurableOwnerDispatchResult> {
    ...
    if (wake.reason === "child_overdue") {
      ...
      const { requestSubagentProgressDelivery } = await import("../agents/subagent-registry.js");
      const result = await requestSubagentProgressDelivery(entry.runId);
      if (result.status === "delivered" || result.status === "not_due") {
        return {
          kind: "delivered",
          evidence: { proofBoundary: "canonical_owner_progress", ownerResult: result.status },
        };
      }
      ...
      return { kind: "deferred", reason: "canonical_owner_progress_failed", retryAfterMs: 30_000 };
    }
    ...
    if (typeof entry.endedAt !== "number" || !entry.outcome) {
      return { kind: "deferred", reason: "canonical_owner_not_terminal", retryAfterMs: 30_000 };
    }
...
export const taskRunsOwnerAdapter: DurableOwnerAdapter = {
  ...
  async dispatchAttention({ wake }): Promise<DurableOwnerDispatchResult> {
    const result = await requestTaskAttentionDelivery({ taskId: wake.sourceRef });
    if (result.status === "missing") {
    ...
    if (result.status === "delivered") {
      return {
        kind: "delivered",
        evidence: {
          proofBoundary: "canonical_task_owner",
    ...
    ? params.store
        ...
        .find(
          (run) =>
            ...
            run.idempotencyKey === fact.requesterRunId,
        )
    : undefined;
  const existing = params.store
    .listWakeObligations({ sourceOwner: fact.sourceOwner, sourceRef: fact.sourceRef, limit: 100 })
    .find((wake) => wake.dedupeKey === fact.dedupeKey);
