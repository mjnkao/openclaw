...
function wakeSourceRevision(wake: WakeObligation): string | undefined {
  if (wake.sourceRevision?.trim()) {
    return wake.sourceRevision;
  }
  const metadataRevision = wake.metadata?.sourceRevision;
...
function gateChangedSourceRevision(params: {
  ...
  const evidence = {
    ...
    currentOccurrenceKey: params.currentFact?.occurrenceKey,
  };
  if (
    !params.currentFact ||
    params.currentFact.sourceOwner !== params.wake.sourceOwner ||
    params.currentFact.sourceRef !== params.wake.sourceRef ||
    params.currentFact.reason !== params.wake.reason ||
    params.currentFact.targetKind !== params.wake.targetKind ||
    params.currentFact.targetRef !== params.wake.targetRef ||
    params.currentFact.ownerRef !== params.wake.ownerRef ||
    params.currentFact.reportRouteRef !== params.wake.reportRouteRef
  ) {
    return {
      kind: "superseded",
...
function subagentAttentionFact(
  ...
  if (entry.execution?.status === "interrupted") {
    const sourceRevision = subagentRevision(entry);
    return {
      ...base,
      reason: "restart_interrupted",
      occurrenceKey: `subagent-interrupted-wake:${entry.runId}:${targetRef}:${sourceRevision}`,
      metadata: {
        sourceRevision,
  ...
  const progressBoundary = entry.startedAt ?? entry.createdAt;
  if (typeof entry.endedAt !== "number" && now - progressBoundary >= SUBAGENT_PROGRESS_SLA_MS) {
    ...
    return {
      ...
      occurrenceKey: `subagent-progress:${entry.runId}:${checkpoint}:${sourceRevision}`,
      metadata: {
        sourceRevision,
        childSessionKey: entry.childSessionKey,
  ...
  return {
    ...
    occurrenceKey: `subagent-terminal:${entry.runId}:${targetRef}:${sourceRevision}`,
...
export const subagentRunsOwnerAdapter: DurableOwnerAdapter = {
  ...
  listAttentionFacts(options): DurableOwnerAttentionFact[] {
    const requestedLimit = options?.limit === undefined ? 500 : Math.max(1, options.limit);
    const candidateLimit = Math.min(5000, Math.max(100, requestedLimit * 4));
    const facts = listSubagentAttentionCandidatesFromSqlite({ limit: candidateLimit }).flatMap(
      (entry) => {
...
function taskAttentionFact(
  ...
  if (active && now - progressBoundary >= TASK_PROGRESS_SLA_MS) {
    const sourceRevision = taskRevision(task);
    return {
      ...base,
      reason: "child_overdue",
      occurrenceKey: `task-progress:${task.taskId}:${Math.floor(now / TASK_PROGRESS_SLA_MS)}:${sourceRevision}`,
      metadata: {
        sourceRevision,
  ...
  return {
    ...
    occurrenceKey: `task-terminal:${task.taskId}:${task.status}:${task.endedAt ?? 0}:${sourceRevision}`,
    metadata: {
      sourceRevision,
      runtime: task.runtime,
      status: task.status,
...
function flowAttentionFact(
  ...
  if (active && now - flow.updatedAt >= FLOW_PROGRESS_SLA_MS) {
    ...
    const sourceRevision = flowRevision(flow);
    return {
      ...base,
      reason,
      occurrenceKey: `flow-attention:${flow.flowId}:${flow.revision}:${Math.floor(now / FLOW_PROGRESS_SLA_MS)}:${sourceRevision}`,
      metadata: {
        sourceRevision,
        status: flow.status,
  ...
  return {
    ...
    occurrenceKey: `flow-terminal:${flow.flowId}:${flow.revision}:${sourceRevision}`,
    metadata: {
      sourceRevision,
      status: flow.status,
      revision: flow.revision,
...
export type DurableOwnerAttentionReconciliationResult = {
  wake: WakeObligation;
  created: boolean;
  suspended: boolean;
  conflict?: {
    reason: ReconcileWakeObligationConflictResult["reason"];
    uncertaintyFactId: string;
  };
};

function durableOwnerReconciliationConflictDigest(params: {
  fact: DurableOwnerAttentionFact;
  conflict: ReconcileWakeObligationConflictResult;
}): string {
  return createHash("sha256")
    .update(
      JSON.stringify({
        sourceOwner: params.fact.sourceOwner,
        sourceRef: params.fact.sourceRef,
        occurrenceKey: params.fact.occurrenceKey,
        reason: params.conflict.reason,
        conflictingWakeIds: params.conflict.conflictingWakeIds.toSorted(),
      }),
    )
    .digest("hex")
    .slice(0, 24);
...
function failClosedOwnerReconciliationConflict(params: {
  store: DurableRuntimeStore;
  fact: DurableOwnerAttentionFact;
  conflict: ReconcileWakeObligationConflictResult;
  factsRef: string;
  now: number;
}): DurableOwnerAttentionReconciliationResult {
  const conflictingWakeIds = params.conflict.conflictingWakeIds.toSorted();
  const digest = durableOwnerReconciliationConflictDigest(params);
  const uncertainty = params.store.recordUncertaintyFact({
    sourceOwner: params.fact.sourceOwner,
    sourceRef: params.fact.sourceRef,
    kind: "requires_owner_decision",
    refId: params.conflict.representativeWake?.wakeId,
    factsRef: params.factsRef,
    dedupeKey: `wake-reconciliation-conflict:${digest}`,
    facts: {
      reconciliationReason: params.conflict.reason,
      occurrenceKey: params.fact.occurrenceKey,
      sourceRevision: params.fact.sourceRevision,
      duplicateScan: params.conflict.duplicateScan,
      candidatePersisted: params.conflict.candidatePersisted,
      conflictingWakeIds,
    },
    metadata: {
      ownerAdapter: params.fact.sourceOwner,
      requiredAction: "inspect_wake_reconciliation_conflict",
    },
    now: params.now,
  });
  const representative = params.conflict.representativeWake;
  if (!representative) {
    throw new Error(
      `Durable owner reconciliation conflict ${params.conflict.reason} has no representative wake; uncertainty ${uncertainty.factId} was recorded`,
    );
  }

  let wake = representative;
  if (wake.status !== "acked" && wake.status !== "superseded") {
      ...
      params.store.suspendWakeObligation({
        ...
        metadata: {
          ...
          reconciliationConflict: {
            reason: params.conflict.reason,
            uncertaintyFactId: uncertainty.factId,
            occurrenceKey: params.fact.occurrenceKey,
            conflictingWakeIds,
          },
        },
    ...
    wake =
      params.store.markWakeObligationDecisionRequired({
        wakeId: wake.wakeId,
        actorKind: "system_worker",
        actorRef: "owner_fact_reconciliation",
        decisionKind: "requires_operator_decision",
        reason: `wake reconciliation conflict: ${params.conflict.reason}`,
  ...
  return {
    wake,
    created: false,
    suspended: wake.status === "suspended",
    conflict: {
      reason: params.conflict.reason,
      uncertaintyFactId: uncertainty.factId,
    },
...
export function reconcileDurableOwnerAttentionFact(params: {
  ...
  const projectedMetadata = { ...fact.metadata };
  const reconciliation = params.store.reconcileWakeObligation({
    candidate: {
  ...
  if (reconciliation.disposition === "conflict") {
    return failClosedOwnerReconciliationConflict({
      store: params.store,
      fact,
  ...
  if (!fact.suspendedReason || wake.status === "acked" || wake.status === "superseded") {
    return {
      wake,
      created: reconciliation.disposition === "created",
      suspended: false,
    };
  }
