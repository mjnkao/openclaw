...
CREATE INDEX IF NOT EXISTS idx_durable_execution_records_report_route
  ON durable_execution_records(report_route_ref, updated_at, runtime_run_id)
...
CREATE TABLE IF NOT EXISTS wake_obligations (
  ...
  coalescing_mode TEXT NOT NULL CHECK (coalescing_mode IN ('none', 'while_unresolved')),
  recurrence_policy TEXT CHECK (recurrence_policy IN ('never', 'after_terminal')),
  parent_run_id TEXT,
  ...
  metadata_json TEXT,
  CHECK (
    (coalescing_mode = 'none' AND recurrence_policy IS NULL) OR
    (coalescing_mode = 'while_unresolved' AND recurrence_policy IS NOT NULL)
  )
);
...
CREATE INDEX IF NOT EXISTS idx_wake_obligations_unresolved_identity
  ON wake_obligations(
    source_owner,
    source_ref,
    coalescing_mode,
...
CREATE INDEX IF NOT EXISTS idx_wake_obligations_terminal_identity
  ON wake_obligations(
    source_owner,
    source_ref,
    coalescing_mode,
...
CREATE TABLE IF NOT EXISTS wake_obligation_occurrences (
  ...
  PRIMARY KEY (source_owner, source_ref, occurrence_key),
  FOREIGN KEY (wake_id) REFERENCES wake_obligations(wake_id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_wake_obligation_occurrences_wake
  ON wake_obligation_occurrences(wake_id, observed_at, occurrence_key);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_uncertainty_facts_source_dedupe
  ON uncertainty_facts(source_owner, source_ref, dedupe_key)
...
CREATE INDEX IF NOT EXISTS idx_delivery_attempt_evidence_wake
  ON delivery_attempt_evidence(wake_id, scheduled_at, delivery_attempt_id);
