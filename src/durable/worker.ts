...
function requireRegisteredOperationScope(
  ...
  if (!normalized) {
    throw new Error("Durable runtime worker requires an operationKind scope");
  }
  if (!registry.getRuntime(normalized)) {
    throw new Error(`Durable runtime worker operation is not registered: ${normalized}`);
  }
  return normalized;
}
...
export function startDurableRuntimeWorker(
  ...
  const operationKind = requireRegisteredOperationScope(options.registry, options.operationKind);
