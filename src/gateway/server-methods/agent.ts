...
function readAgentRunYieldState(result: unknown): {
  ...
  let livenessState = readString(meta?.livenessState);
  let openclawProgressKind =
    readString(meta?.openclawProgressKind) ?? readString(meta?.progressKind);
  ...
  for (const payload of payloads) {
    ...
    livenessState ??= readString(channelData?.livenessState);
    openclawProgressKind ??=
      readString(channelData?.openclawProgressKind) ?? readString(channelData?.progressKind);
  }
  return {
  ...
  void agentCommandFromIngress(params.ingressOpts, defaultRuntime, params.context.deps)
    .then((result) => {
      ...
      params.durableLifecycle?.markTerminal({
        ...
        eventType: durableTerminal.eventType,
        payload: {
          summary:
            durableTerminal.status === "failed"
              ? (yieldState.livenessState ?? "failed")
              : payload.summary,
